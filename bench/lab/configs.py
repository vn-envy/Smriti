...
def _ev(name, **recall):
    ...
    # The profile keeps the shipped name "evidence" so variants get exactly the
    # default read path (e.g. aggregation routing keyed on that name) plus their
    # overrides; the system label carries the variant spec.
    prof = RetrievalProfile(name="evidence", engine="evidence", recall=RecallConfig(**recall))
