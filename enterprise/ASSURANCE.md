...
| Untrusted-content boundary | partial | immutable `origin` at write + mutable `quarantined` review state + `strict=True` retrieval filter on `search()` (facts and, since 0.2.0, raw turns) | `test_strict_profile_excludes_quarantined_and_untrusted`, `test_strict_search_drops_untrusted_raw_turns` | origin is assigned by the calling host; a caller-declared label is only as trustworthy as the caller; `context()` has no strict mode, so build prompts for consequential actions from `search(strict=True)` results |
